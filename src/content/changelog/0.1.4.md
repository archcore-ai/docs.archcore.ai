---
version: "0.1.4"
date: 2026-04-04
title: "Tags — Cross-Cutting Labels for Documents"
description: "Adds optional tags to document frontmatter, with a tags filter in list_documents and tag hygiene in archcore doctor."
product: "cli"
---

This release adds **tags** to document frontmatter: optional labels that categorize documents across directories, teams, and concerns.

## Improvements

- Tags in document frontmatter: lowercase labels (`^[a-z][a-z0-9_:|-]*$`) with colon-namespaced conventions like `team:payments` or `scope:frontend`
- `list_documents` gains a `tags` filter parameter with OR semantics: a document matches if it carries at least one of the given tags
- `create_document` and `update_document` accept an optional `tags` array. Omit it to preserve tags, pass `[]` to clear them, pass values to replace them
- Session hooks inject `EXISTING TAGS: ...` (top 30 by frequency) so agents see available tags without asking
- `archcore doctor` reports tag hygiene: invalid format, singleton tags (possible typos), and unique tag count

## Misc

- Updated context documentation for tags
- Added Golang context and code review skill

## Meet tags: one document, many contexts

Directory structure is one-dimensional. A document like `e2e-auth-flow.rule.md` lives in one folder, but it matters to the frontend team, the backend team, and the QA guild. Tags give you a second axis of organization without moving files.

**When to use them:**

- Marking which team owns or cares about a document: `team:backend`, `team:payments`
- Grouping documents by concern across directories: `auth`, `billing`, `onboarding`
- Filtering context for focused agent sessions: "show me everything tagged `api`"

**Quick test:** _"Would someone outside this folder's natural audience need to find this document?"_ If yes, add a tag.

**Example: tagging a cross-team decision**

```bash
archcore mcp
# Then ask your agent:
# "Create an ADR about our new auth token rotation strategy
#  and tag it with auth, team:backend, and team:frontend"
```

Your agent creates `.archcore/auth-token-rotation.adr.md` with tags in the frontmatter:

```yaml
---
title: "Auth Token Rotation Strategy"
status: accepted
tags:
  - auth
  - team:backend
  - team:frontend
---
```

Now anyone working on auth, backend, or frontend sees this decision in their filtered view.

**Example: filtering documents by tag**

```bash
# "Show me all documents tagged with team:backend"
# "List every document tagged auth or billing"
```

The `list_documents` tool returns only matching documents. With OR semantics, a filter on `auth` or `billing` returns documents that carry either tag. Use it to pull cross-cutting context into a focused session.

**Example: updating tags on an existing document**

```bash
# "Add the payment_flow tag to our Stripe integration ADR"
# "Remove all tags from the legacy migration plan"
```

Pass a new array to replace tags entirely, or pass `[]` to clear them. Omit the field to leave existing tags untouched.

**Tags vs directories:**

| Need                             | Use                             |
| -------------------------------- | ------------------------------- |
| Permanent home for a document    | Directory structure             |
| Cross-cutting team ownership     | `team:backend`, `team:frontend` |
| Thematic grouping across folders | `auth`, `billing`, `onboarding` |
| Temporary project membership     | `project:q2-migration`          |

Colons in tag names are a convention, not enforced structure. Use them for namespaces like `team:`, `scope:`, or `project:`.

<a href="https://github.com/archcore-ai/cli/releases/tag/v0.1.4" target="_blank" rel="noopener">View on GitHub &rarr;</a>
