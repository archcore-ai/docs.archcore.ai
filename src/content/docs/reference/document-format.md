...
Only the frontmatter is validated when a document is written: `title` must be present and non-empty, and `status` must be `draft`, `accepted`, or `rejected`. The body is measured separately by the [precision checks](/reference/precision-checks/), which report a missing section, a vague word, or a misplaced requirement after the write. Those findings are advisory and never reject a document.
