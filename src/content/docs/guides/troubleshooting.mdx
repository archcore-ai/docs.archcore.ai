...
**Why it happens:** The type (the middle part of `slug.type.md`) must be one of the 23 valid types.
...
**Fix:** Rename the file using one of the 23 valid types. See [Document types](/concepts/document-types/) for guidance on choosing.
...
2. **`.codex.mcp.json` exists at plugin root.** It should ship with the marketplace install. If missing, reinstall: `codex plugin uninstall archcore`, then `codex plugin marketplace add archcore-ai/archcore` and re-install Archcore from `/plugins`.
