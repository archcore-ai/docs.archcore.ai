...
These issues appear when you run the plugin inside Claude Code, Cursor, or Codex CLI. For CLI-level issues (MCP not reachable, `.archcore/` missing, validation errors), see [CLI troubleshooting](/cli/troubleshooting/).
...
The plugin ships `.mcp.json` at its root, which runs `archcore mcp` from your `PATH`. If MCP is unreachable, check these causes:
