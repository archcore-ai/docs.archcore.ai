...
The advisory is rate-limited to once per 24h and only fires on the strict parser's exact field-rejection signature, so a current CLI that understands `globals` never triggers it — even in projects that declare globals.
...
With a current CLI, a missing global doesn't block your session — the hook degrades to a **local-only** context summary and names the missing source so your own documents are never silently dropped. (The MCP server itself still fails fast on a missing global; see [CLI troubleshooting](/cli/troubleshooting/#missing-or-invalid-global-source).)
