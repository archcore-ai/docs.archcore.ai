...
Cursor does not auto-register plugin-provided MCP. Copy `cursor.mcp.json` from the installed plugin into `~/.cursor/mcp.json` (user-scoped) or `.cursor/mcp.json` (project-scoped):
...
**Keep `cwd: "${workspaceFolder}"`** — without it, a single registration leaks one project's `.archcore/` into every other Cursor window. Verify the CLI itself resolves:
...
Your Cursor MCP registration is missing the `cwd: "${workspaceFolder}"` line (or has a literal path baked in). Without it, Cursor reuses the first registration across every window and the MCP server reads whichever `.archcore/` it sees from that initial cwd. Re-copy the template from `cursor.mcp.json` at the installed plugin root and restart Cursor.
