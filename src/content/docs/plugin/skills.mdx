...
/archcore:plan research vector store landscape
/archcore:plan rnd vector store options
/archcore:plan evidence https://example.com/benchmark-report
```
...
The skill classifies your request and runs one of two tracks. Name a type in the invocation (`adr`, `rfc`, `spec`, `doc`, `guide`, `rule`, `research`, `evidence`) to skip classification. `document research` files a ready investigation report as a `research`; `document evidence` files one external material as an `evidence`.
...
- [Document types](/concepts/document-types/) lists the 21 types these commands route to.
