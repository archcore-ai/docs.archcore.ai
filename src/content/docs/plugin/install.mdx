...
<Tabs>
	...
	<TabItem label="Codex CLI">
		...
		Before Codex 0.129.0 the key is spelled `codex_hooks = true`. Without this feature, skills, MCP tools, and agents work normally, but the write guard, code-alignment injection, and post-write checks won't fire.
	</TabItem>
	...
	<TabItem label="Local development (any host)">
		...
		**Cursor** has no `--plugin-dir` flag. Symlink the repo into Cursor's local plugins directory, then reload the window:
		...
		If you want hooks active, launch with `codex --enable hooks`, or add `[features]` with `hooks = true` to `~/.codex/config.toml` (`codex_hooks = true` before Codex 0.129.0).
