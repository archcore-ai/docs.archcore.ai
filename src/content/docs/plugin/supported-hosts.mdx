...
| **Cursor**       | Implemented  | Plugin marketplace / local dir | User-registered from `cursor.mcp.json` template (pin `cwd: "${workspaceFolder}"`) | `sessionStart`, `preToolUse`, `postToolUse`, `afterMCPExecution` |
...
| Aspect              | Claude Code                                       | Cursor                                                  | Codex CLI                                                |
| ------------------- | ------------------------------------------------- | ------------------------------------------------------- | -------------------------------------------------------- |
| Who registers MCP   | The plugin, via `.mcp.json` at plugin root        | The user, via `~/.cursor/mcp.json` or `.cursor/mcp.json` (copy `cursor.mcp.json` template) | The plugin, via `.codex.mcp.json` referenced from manifest's `mcpServers` |
| Command points at   | `archcore mcp` (from `PATH`)                      | `archcore mcp` (from `PATH`), with `cwd: "${workspaceFolder}"` and `env.ARCHCORE_CWD` | `archcore mcp` (from `PATH`; Codex inherits shell `PATH`) |
