...
The launcher path is host-relative: Claude Code resolves it as `${CLAUDE_PLUGIN_ROOT}/bin/archcore`, Cursor as `${CURSOR_PLUGIN_ROOT}/bin/archcore`, and Codex CLI uses the host-neutral `${PLUGIN_ROOT}/bin/archcore`.
