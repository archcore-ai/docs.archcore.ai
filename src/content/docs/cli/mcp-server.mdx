...
:::note
These MCP prompts are **different** from the plugin's `/archcore:product-track` slash commands. MCP prompts ship in the CLI's MCP server and work in any MCP-aware client (Gemini CLI, Copilot, Codex, OpenCode, ...). Plugin tracks are skills that ship with the [Archcore Plugin](/plugin/tracks/) and are scoped to plugin hosts.
:::
