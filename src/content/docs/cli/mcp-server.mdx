...
| Search        | `search_documents`    | Word and path-reference search across bodies           |
...
- The type catalog: all 21 [document types](/concepts/document-types/) with their virtual category, including `research` in vision and `evidence` in knowledge
...
:::note
The instructions describe document **types**, not workflows. The Archcore plugin's [`/archcore:plan`](/plugin/skills/#archcoreplan) skill assembles multi-document packages, including the `sdd`, `sources`, and `iso` expert paths, so workflow orchestration has a single owner. The MCP server dropped its five track prompts in v0.7.0, and `prompts/list` now answers with a JSON-RPC method-not-found.
:::
...
Otherwise the server keeps the current root and writes one line to stderr per distinct reason; [CLI troubleshooting](/cli/troubleshooting/#wrong-working-directory) lists the reasons. A tool call never fails because of the query. The instructions the server returns at `initialize` come from the root at start time. `--project <path>` or `ARCHCORE_PROJECT_ROOT` pins the root for the process lifetime and turns the query off.
