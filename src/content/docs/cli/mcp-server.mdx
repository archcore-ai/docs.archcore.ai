...
:::note
The instructions describe document **types**, not workflows. The Archcore plugin's [`/archcore:plan --track`](/plugin/skills/#archcoreplan) skill drives multi-document track cascades such as PRD → Plan and the ISO cascade, so track orchestration has a single owner. The MCP server dropped its five track prompts in v0.7.0, and `prompts/list` now answers with a JSON-RPC method-not-found.
:::
