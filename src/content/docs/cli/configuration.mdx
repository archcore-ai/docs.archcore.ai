...
`archcore mcp`, `archcore config`, `archcore doctor`, and `archcore sync` warn about unrecognized fields on stderr, so `config get` output and the MCP server's stdout stay clean:
