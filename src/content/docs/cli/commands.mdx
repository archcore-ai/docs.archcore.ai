...
- Global source health: any [fatal state](/cli/global-sources/#every-declared-global-is-mandatory) (missing, not a directory, unreadable, self-overlap, duplicate path) is a failure, an empty source is a warning, and a healthy source prints `global source "<id>" (<N> document(s))`
