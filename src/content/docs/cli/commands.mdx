...
If no agents are detected, behavior depends on the environment. In an interactive terminal, `init` prompts a multi-select of all supported agents plus a "Skip — configure later" option. In non-interactive contexts (no TTY), it installs nothing and prints a hint to run `archcore mcp install --agent <id>` later.

Re-running `init` prompts "Reinitialize?". Confirming overwrites `.archcore/settings.json` back to `{"sync":"none"}`, discarding any `language` or `globals` you had set; declining leaves it untouched. MCP and hook writes are always idempotent.

In non-interactive contexts (no TTY) the instruction hint is skipped — add it later with [`archcore instructions install`](#archcore-instructions).
