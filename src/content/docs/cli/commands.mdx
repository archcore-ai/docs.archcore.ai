...
`init` also delivers the [Archcore plugin](/guides/commands/), and an explicit host selection is the only thing that authorizes it.
...
For a step-by-step walkthrough, see the [CLI Quick Start](/start/quick-start/).
...
- Document types are valid (one of the 23 types)
...
- Global source health: any [fatal state](/cli/configuration/#every-declared-global-is-mandatory) (missing, not a directory, unreadable, self-overlap, duplicate path) is a failure, an empty source is a warning, and a healthy source prints `global source "<id>" (<N> document(s))`
...
`doctor` runs every [`archcore status`](#archcore-status) check, tag hygiene included, plus:
...
See [Configuration](/cli/configuration/#code-alignment) for the default source roots and the validation rules.
...
See [MCP server](/guides/connect-your-agent/#mcp) for details.
...
See [Hooks](/guides/connect-your-agent/#lifecycle-hooks) for the full guard and advisory detail.
...
Three frozen identifiers address the plugin everywhere: repository `archcore-ai/archcore`, marketplace `archcore-plugins`, plugin id `archcore@archcore-plugins`.
...
| Host           | Commands                                                                                                                                                                                                        |
| -------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code    | `claude plugin marketplace add archcore-ai/archcore`, then `claude plugin install archcore@archcore-plugins` (user scope by default), then merges an `extraKnownMarketplaces["archcore-plugins"]` entry with `"autoUpdate": true` into `~/.claude/settings.json`, preserving unknown fields |
...
| Codex CLI      | `codex plugin marketplace add archcore-ai/archcore`, then `codex plugin add archcore@archcore-plugins` (machine-level)                                                                                           |
| GitHub Copilot | `copilot plugin install archcore-ai/archcore:plugins/archcore` (machine-level)                                                                                                                                   |
...
For installing the plugin from a host's own UI or CLI, see [Install the plugin](/start/install/).
...
Reads the latest release tag from the `https://github.com/archcore-ai/archcore/releases/latest` redirect (not the GitHub API), compares versions, downloads the binary for your OS/architecture, verifies the checksum, and updates in place.
...
- Set `DO_NOT_TRACK` or `ARCHCORE_TELEMETRY_OPTOUT` to any value other than `0` to stop every event. These are the same two variables the [analytics and opt-out](/start/install/#analytics-and-opt-out) section describes.
