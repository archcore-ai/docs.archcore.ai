...
| Block direct writes to `.archcore/*.md`           | Yes              | Registers the hook only (delegates to the CLI) |
| Detect cascade staleness after `update_document`  | Yes              | Registers the hook only (delegates to the CLI) |
