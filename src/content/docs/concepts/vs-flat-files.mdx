...
For the category-level version of this comparison — repo memory against cloud agent memory, RAG, and instruction files — see [What is repo memory?](https://archcore.ai/learn/repo-memory/).
...
In a flat file, a decision and a coding standard are just paragraphs. Archcore gives each piece of knowledge an explicit type: `adr` for decisions, `rule` for standards, `guide` for how-tos, `plan` for implementation tasks — [19 types](/concepts/document-types/) total. Each type has a template. Agents query by type instead of parsing everything.
...
One `.archcore/` directory works with [Claude Code, Cursor, Copilot, Gemini CLI, and more](/cli/agent-integrations/). Agents don't just read — they query by type and status, create documents from templates, and add relations through [MCP](/cli/mcp-server/).
...
- [Plugin quick start](/start/plugin-quick-start/) — get the full UX in Claude Code or Cursor.
- [CLI quick start](/cli/quick-start/) — install the CLI and create your first document.
- [Document Types](/concepts/document-types/) — the 19 types and when to use each.
