...
Archcore gives your team a single place to capture decisions, standards, plans, and project knowledge — all as structured markdown files that every AI agent discovers automatically through MCP.

Ready to set it up? Pick your entry point: the [plugin](https://archcore.ai/plugin/) for Claude Code, Cursor, and Codex CLI, or the [CLI](https://archcore.ai/cli/) for any MCP-aware agent.
