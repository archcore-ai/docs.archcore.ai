...
**Why it happens:** The type (the middle part of `slug.type.md`) must be one of the 10 valid types. Anything else is rejected.
...
| `api-spec.reference.md` | `reference` is not a type | Rename to `api-spec.doc.md` |

**Fix:** Rename the file using one of the 10 valid types. See [Document Types](/concepts/document-types/) for guidance on choosing the right type.
