...
Flat files hit a wall around 50–200 lines. See [Flat files vs Archcore](/concepts/vs-flat-files/) for the full comparison. Short version:
...
**Using the plugin?** Install the CLI first (see [CLI install](/cli/install/)), then follow the [Plugin quick start](/start/plugin-quick-start/) — the plugin initializes `.archcore/` through MCP on first call, no manual `archcore init` needed. The [plugin page](https://archcore.ai/plugin/) lists which hosts it runs in.
...
Start where agents will feel the difference on the next session.
...
If both are green, agents can see and follow the new content. Start a fresh session and ask:
