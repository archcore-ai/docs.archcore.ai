...
**Using the plugin in Claude Code?** Skip the CLI install — follow the [Plugin quick start](/start/plugin-quick-start/). The plugin bundles a launcher that auto-downloads the CLI on first use and initializes `.archcore/` through MCP. Zero setup.

**Otherwise** (Cursor, any non-plugin host, scripting, CI), install the CLI directly:
