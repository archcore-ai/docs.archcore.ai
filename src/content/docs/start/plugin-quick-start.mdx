...
<Tabs>
	...
	<TabItem label="Codex CLI">
		...
		MCP is auto-registered via the plugin's `.codex.mcp.json`. Enable Codex's `hooks` feature for full coverage, with `codex --enable hooks` or in `~/.codex/config.toml`:
		...
		Before Codex 0.129.0 the key is spelled `codex_hooks = true`. Without the feature, commands, MCP tools, and agents work normally, but the write guard, code-alignment injection, and post-write checks won't fire.
	</TabItem>
