...
The default `sdd` track walks you through an idea, a PRD, a spec, and a plan, one document per gate, with the relations wired between them. You review each gate before the next opens. Name a different track when you need a different depth:

- `sources` (mrd → brd → urd): market research and discovery.
- `iso` (brs → strs → syrs → srs): formal ISO 29148 cascade.

Name the track first: `/archcore:plan iso payment gateway compliance`.
